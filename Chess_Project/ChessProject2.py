        ...
        if self.color == 'white':
            if white_king.check:
                ...
                if isinstance(white_king.check_figure, Bishop):
                    ...
                    for step in self.steps[0][1:]:     
                        if not get_avaible(white_king.check_figure.coord, (step[0] + 25, step[1] + 10), 'Pawn'):
                            new_steps[0].append(step)
                    for step in self.steps[1]:
                        if white_king.check_figure.coord == (step[0] + 25, step[1] + 10):
                            new_steps[1].append(step) 
            ...
            if black_king.check:
                ...
                if isinstance(black_king.check_figure, Bishop):
                    for step in self.steps[0][1:]:     
                        if not get_avaible(black_king.check_figure.coord, (step[0] + 25, step[1] + 10), 'Bishop'):
                            new_steps[0].append(step)
                    for step in self.steps[1]:
                        if black_king.check_figure.coord == (step[0] + 25, step[1] + 10):
                            new_steps[1].append(step)
                    ...
                    for step in self.steps[0][1:]:     
                        if not get_avaible(black_king.check_figure.coord, (step[0] + 25, step[1] + 10), 'Pawn', 'black'):
                            new_steps[0].append(step)
                    for step in self.steps[1]:
                        if black_king.check_figure.coord == (step[0] + 25, step[1] + 10):
                            new_steps[1].append(step)                    
...
def update_figures():
    ...
    if len(white_figures_steps) == 0 and white_king.check == True:
        black_mate.show()
    elif len(white_figures_steps) == 0 and white_king.check == False:
        draw.show()
    for black_fig in black_figures + [black_king]:
        black_fig.update_steps()
        black_figures_steps += black_fig.steps[0][1:] + black_fig.steps[1]
    if len(black_figures_steps) == 0 and black_king.check == True:
        white_mate.show()
    elif len(black_figures_steps) == 0 and black_king.check == False:
        draw.show()
...
class Draw:
    def __init__(self):
        self.punkts = [((450, 50), image.load(r'Punkts\quit.png'), image.load(r'Punkts\selected_quit.png'), 0)]
        self.base = image.load(r'PS\draw.png')
    def render(self, num_punkt = None):
        if num_punkt != None:
            for i in self.punkts:
                if i[3] == num_punkt:
            ...
            if e.button == 1:                
                ...
                for i in dosk:
                    ...
                    if mp[0] > i[0] - 25 and mp[0] < i[0] + 75 and mp[1] > i[1] - 10 and mp[1] < i[1] + 90:
                        if dosk[i] != None or (spawn and (i[0] - 25, i[1] - 10) in active_figure.steps[1]):
                            if not spawn:
